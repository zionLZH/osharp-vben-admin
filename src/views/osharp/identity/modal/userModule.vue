<template>
  <BasicModal
    ...
    @ok="doSubmit"
    :showOkBtn="hasPermission('User.SetModules')"
    width="800px"
...
<script lang="ts" setup>
  ...
  import { ReadUserModules } from '/@/api/osharp/Module'
  import { SetModules } from '/@/api/osharp/User'
  ...
  async function getDetail() {
    ...
    try {
      ...
      const res = await ReadUserModules(roleId)
      data.value = res
      await nextTick()
      let arr: Array<any> = []
      for (let item of res) {
  ...
  async function doSubmit() {
    ...
    try {
      ...
      await SetModules({ userId, moduleIds })
